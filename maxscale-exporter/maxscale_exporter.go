...
var (
	...
	monitorLabelNames  = []string{"id", "node"}
)
...
var (
	...
	monitorMetrics = metrics{
	    "seconds_behing_master":   newDesc("monitor", "seconds_behing_master", "seconds behing master", monitorLabelNames, prometheus.GaugeValue),
		"gtid_current_pos":        newDesc("monitor", "gtid_current_pos", "gtid current pos", monitorLabelNames, prometheus.GaugeValue),
	}
)
...
func (m *MaxScale) Describe(ch chan<- *prometheus.Desc) {
	...
	for _, m := range m.monitorMetrics {
		ch <- m.Desc
	}
...
func (m *MaxScale) parseMonitors(ch chan<- prometheus.Metric) error {
    ...
    monitors := []MonitorList{
		MonitorList{"seconds_behing_master"},
		MonitorList{"gtid_current_pos"},
	}

	body := m.getStatistics("/monitors")

	jsonparser.ArrayEach(body, func(value []byte, dataType jsonparser.ValueType, offset int, err error) {

		// Get id monitor
		idMonitor, err := jsonparser.GetString(value, "id")

		if err != nil {
			fmt.Println(err)
		}

		// Loop in ["attributes"]["monitor_diagnostics"] of a monitor
		jsonparser.ObjectEach(value, func(key []byte, value2 []byte, dataType jsonparser.ValueType, offset int) error {
            if string(key) == "server_info" {

                // Loop in server_info
                jsonparser.ArrayEach(value2, func(value3 []byte, dataType jsonparser.ValueType, offset int, err error) {

                    // get id node
                    nameNode, _ := jsonparser.GetString(value3, "name")

                    // loop in objects of server_info
                    jsonparser.ObjectEach(value3, func(key2 []byte, value4 []byte, dataType jsonparser.ValueType, offset int) error {
                        for _, monitor := range monitors {
                            if string(key2) == string(monitor.Name) {
                                floatTempNode, err = strconv.ParseFloat(strings.Replace(string(value4), "-", "", -1), 64)
                                if err != nil {
                                    fmt.Println(err)
                                }

                                monitorInfoConnectionsMetric := m.monitorMetrics[monitor.Name]
                                ch <- prometheus.MustNewConstMetric(
                                    monitorInfoConnectionsMetric.Desc,
                                    monitorInfoConnectionsMetric.ValueType,
                                    floatTempNode,
                                    idMonitor, nameNode,
                                )

                            }
                            if string(key2) == "slave_connections" {
                                // Loop in slave_connections
                                jsonparser.ArrayEach(value4, func(value5 []byte, dataType jsonparser.ValueType, offset int, err error) {
                                    jsonparser.ObjectEach(value5, func(key3 []byte, value6 []byte, dataType jsonparser.ValueType, offset int) error {
                                        for _, monitor := range monitors {
                                            if string(key3) == string(monitor.Name) {
                                                floatTempNode, err = strconv.ParseFloat(string(value6), 64)
                                                if err != nil {
                                                    fmt.Println(err)
                                                }

                                                monitorConnectionsMetric := m.monitorMetrics[monitor.Name]
                                                ch <- prometheus.MustNewConstMetric(
                                                    monitorConnectionsMetric.Desc,
                                                    monitorConnectionsMetric.ValueType,
                                                    floatTempNode,
                                                    idMonitor, nameNode,
                                                )
			...
			return nil
		}, "attributes", "monitor_diagnostics")
